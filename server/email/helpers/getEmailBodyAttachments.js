...
export default function getEmailBodyAttachments(emailAddress, selectedOptions) {
  ...
  for (var i = 0; i < applicationDocsGeneral.length; i++) {
    ...
    if (applicationDocsGeneral[i].docTemplate != null) {
      ...
      console.log("applicationDocsGeneral[i].docName", applicationDocsGeneral[i].docName);
      console.log("applicationDocsGeneral[i].docTemplate", applicationDocsGeneral[i].docTemplate);
      attachementsArray.push({
        filename: applicationDocsGeneral[i].docTemplate,
        path: path.join(process.cwd(), 'server/email/docs/' + applicationDocsGeneral[i].docTemplate),
        encoding: "UTF-8"
      })
  ...
  for (var j = 0; j < applicationDocsSpecfic.length; j++) {
    ...
    if (applicationDocsSpecfic[j].docTemplate !== null) {
      htmlReturnString += ' (template attached)'
      console.log("applicationDocsSpecfic[j].docName", applicationDocsSpecfic[j].docName);
      console.log("applicationDocsSpecfic[j].docTemplate", applicationDocsSpecfic[j].docTemplate);
      attachementsArray.push({
        filename: applicationDocsSpecfic[j].docTemplate,
        path: path.join(process.cwd(), 'server/email/docs/' + applicationDocsSpecfic[j].docTemplate),
        encoding: "UTF-8"
      })
    }
    htmlReturnString += '</div>'
  ...
  htmlReturnString += '<br/>'
  htmlReturnString += '<div>Send above documents to Hussain Bhai Ghani in Tawfeer Office or email to: mohammed.petiwala52@gmail.com</div>'
  htmlReturnString += '<br/>'
